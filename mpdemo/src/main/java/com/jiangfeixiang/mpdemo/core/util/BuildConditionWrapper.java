...
// 自定义①SQL连接类型（AND还是OR）② 自定义基础异常类；③ 条件构造VO
import com.jiangfeixiang.mpdemo.core.enums.JoinType;
import com.jiangfeixiang.mpdemo.core.exception.BaseException;
import com.jiangfeixiang.mpdemo.core.vo.Condition;
...
public class BuildConditionWrapper {
    ...
    /**
     * 此条件构建包装器方法是支持多个表组装成SQL字段的虚拟表，不支持实际存在的表
     */
    public static <T> QueryWrapper<T> buildWrapper(List<Condition> conditions) {
        //初始化mybatis条件构造器
        QueryWrapper wrapper = Wrappers.query();
        if (conditions == null || conditions.size() == 0) {
            return wrapper;
